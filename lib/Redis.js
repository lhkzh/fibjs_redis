        ...
        M: while (!self.killed && handler.check(sock)) {
            try {
                ...
                if (line[0] == '+') {
                    ...
                    let len = Number(line.substr(1)), arr = new Array(len), subLine, subRaw;
                    for (var i = 0; i < len; i++) {
...
class Redis {
    ...
    _pre_Fibers() {
        ...
        T._sender = coroutine.start(function () {
            let self = T, sock = self._socket, buf, evt = self._waitWrite.event, tcs;
    ...
    pipelineSubmitBridge(commands, casts) {
        this.assertNomal();
        this.beforeSend();
